...
api_token_path = os.path.join(node_dir, '.api-token.txt')
...
def load_token():
    ...
    # Check if token already exist in environment variable
    if TOKEN:
        return
    
    # Check if there is token inside a file
    if os.path.exists(api_token_path):
        with open(api_token_path, "r") as f:
            TOKEN = f.read().strip()
            logging.info(f"Loaded recorded api token")
            return

    # If no token was found, generate random api token and record it
